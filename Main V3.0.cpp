...
void flipimage()
{
    ...
    while (true)
    {
        cout << "1- flip image vertically, enter (V) " << endl;
        cout << "2- flip image horizontally , enter (h)" << endl;
        cin >> type;
        if (type == 'h')
        {
            flipimageH();
...
void DetectimageRGB()
{
    ...
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            for (int k = 0; k < RGB; k = k + 3)
            {
                if (colored_image[i][j][k] + colored_image[i][j][k + 1] + colored_image[i][j][k + 2] >= 382)
                {
                    colored_image[i][j][k] = 255;
                    colored_image[i][j][k + 1] = 255;
                    colored_image[i][j][k + 2] = 255;
                }
                else
                {
                    colored_image[i][j][k] = 0;
                    colored_image[i][j][k + 1] = 0;
                    colored_image[i][j][k + 2] = 0;
                }
            }
        }
    }
    ...
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            ...
            if (x == 0)
            {
                if (colored_image[i][j][0] == 0)
                {
                    detectimage[i][j][0] = 0;
                    detectimage[i][j][1] = 0;
                    detectimage[i][j][2] = 0;
                    x = 1;
                    if (colored_image[i - 1][j][0] == 255 or colored_image[i + 1][j][0] == 255 or colored_image[i - 1][j + 1][0] == 255 or colored_image[i - 1][j - 1][0] == 255 or colored_image[i + 1][j + 1][0] == 255 or colored_image[i + 1][j - 1][0] == 255)
                    {
                        detectimage[i][j][0] = 0;
                        detectimage[i][j][1] = 0;
                        detectimage[i][j][2] = 0;
    ...
    for (int i = 0; i < SIZE; i++)
    {
        ...
        {
            for (int k = 0; k < RGB; k++)
                colored_image[i][j][k] = detectimage[i][j][k];
        }
    }
}
void down_side()
{
    ...
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            for (int k = 0; k < RGB; k++)
            {
                colored_image[i][j][k] = colored_image[SIZE - i][j][k];
            }
        }
    }
}
void right_side()
{
    ...
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            for (int k = 0; k < RGB; k++)
            {
                colored_image[i][j][k] = colored_image[i][SIZE - j][k];
            }
        }
    }
}
void upper_side()
{
    ...
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            for (int k = 0; k < RGB; k++)
            {
                colored_image[SIZE - i][j][k] = colored_image[i][j][k];
            }
        }
    }
}
void left_side()
{
    ...
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            for (int k = 0; k < RGB; k++)
            {
                colored_image[i][SIZE - j][k] = colored_image[i][j][k];
            }
        }
    }
}
